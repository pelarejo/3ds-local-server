    ...
    def add_string_array(self, values: Iterable[str]) -> int:
        ...
        for value in values:
            ...
            if not raw or b"\0" in raw:
                raise ValueError("NB string-array entries must be non-empty and cannot contain NUL bytes")
            encoded.append(raw + b"\0")
        if not encoded:
            return 0
        payload = b"\0\0\0\0" + b"".join(encoded)
        payload += b"\0" * (_aligned(len(payload)) - len(payload))
        return self.add_bytes(struct.pack("<4sIII", b"NBRA", 16, len(encoded), len(payload)) + payload)
...
def single_object(magic: bytes, header: bytes, blob: bytes | bytearray) -> bytes:
    ...
    return struct.pack("<4sIII", magic, 16, len(header), len(blob)) + header + bytes(blob)


def object_array(headers: Iterable[bytes], blob: bytes | bytearray, element_size: int) -> bytes:
    ...
    return struct.pack("<4sIIII", b"NBAR", 20, len(headers), element_size, len(blob)) + b"".join(headers) + bytes(blob)


def result_payload(namespace: ResultNamespace, reason: ResultReason, message: str) -> bytes:
    blob = Blob()
...
def result_response(namespace: ResultNamespace, reason: ResultReason, message: str, *, status: int) -> HttpResponse:
    return HttpResponse(result_payload(namespace, reason, message), status=status, content_type=NB_CONTENT_TYPE)


def versioned_response(payload: bytes, *, status: int = 200, content_type: str = NB_CONTENT_TYPE) -> HttpResponse:
...
def title_payload(title) -> bytes:
    ...
    struct.pack_into("<QQQQQQ", header, 16, title.artifact_size, title.title_id_int,
                     int(title.added_at.timestamp()), int(title.updated_at.timestamp()),
                     title.download_count, title.flags)
    struct.pack_into("<IIIIIII", header, 64, title.pk, blob.add_string(title.name),
                     blob.add_string(title.alternative_name), blob.add_string(title.region),
                     blob.add_string(title.filename), blob.add_string(title.description),
                     blob.add_string(title.product_code))
    struct.pack_into("<HBBBB", header, 92, title.version, title.content_type,
                     title.category.protocol_id, title.subcategory.protocol_id, title.listed)
    struct.pack_into("<II", header, 100, blob.add_string_array(title.alternative_names or []),
                     title.preferred_alternative_index)
...
def token_payload(grant, raw_token: str) -> bytes:
    blob = Blob()
    header = struct.pack("<QII", int(grant.expires_at.timestamp()), grant.artifact.title_id, blob.add_string(raw_token))
    return single_object(b"TOKN", header, blob.data)


def _index_meta(titles) -> bytes:
    title_list = list(titles)
    return struct.pack("<III4xQQ", len(title_list), len(title_list),
...
def index_payload(categories, generated_at: int) -> bytes:
    ...
    for category in categories:
        category_titles = list(category.titles.filter(listed=True).select_related("artifact"))
        all_titles.extend(category_titles)
        sub_blob = Blob()
        sub_headers = []
        for subcategory in category.subcategories.all():
            sub_titles = [title for title in category_titles if title.subcategory_id == subcategory.pk]
            sub_headers.append(
                _index_meta(sub_titles)
                + struct.pack("<IIII", subcategory.protocol_id, sub_blob.add_string(subcategory.display_name),
                              sub_blob.add_string(subcategory.slug), sub_blob.add_string(subcategory.description))
            )
        sub_array_offset = category_blob.add_bytes(object_array(sub_headers, sub_blob.data, 48)) if sub_headers else 0
        category_headers.append(
            _index_meta(category_titles)
            + struct.pack("<IIIIB3xI", category.protocol_id, category_blob.add_string(category.display_name),
                          category_blob.add_string(category.slug), category_blob.add_string(category.description),
                          category.priority, sub_array_offset)
        )
    categories_offset = outer_blob.add_bytes(object_array(category_headers, category_blob.data, 56)) if category_headers else 0
    index_header = _index_meta(all_titles) + struct.pack("<I4xQ", categories_offset, generated_at)
